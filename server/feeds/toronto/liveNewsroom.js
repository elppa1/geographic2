...
async function fetchFireSnapshot() {
  // Toronto's LiveCAD XML sits behind web caching. A browser-side
  // `cache: 'no-store'` hint is not enough to guarantee that an
  // intermediary/CDN gives us the newest CAD snapshot. Give every
  // poll a unique URL and explicit no-cache headers so major Fire
  // incidents cannot sit behind a stale XML response for hours.
  const requestUrl =
    (
      FIRE_UPSTREAM +
      '?geographic=' +
  ...
  const sourceUpdatedAt =
    decodeXmlText(
      String(
        ...
        .match(
          /<update_from_db_time\b[^>]*>([\s\S]*?)<\/update_from_db_time>/i
        )?.[1] ||
        ''
    )
