...
function parseTorontoFireTime(
  ...
  // Toronto Fire dispatch_time is a Toronto wall-clock value without
  // a timezone offset. Convert this Fire field only.
  const match =
    clean.match(
  ...
  // Preserve explicitly-zoned values if Toronto ever adds an offset.
  if (
    /(?:Z|[+-]\d{2}:?\d{2})$/i.test(
      clean
    )
...
function formatFireDispatchTime(
  value
) {
  const match =
    cleanText(
...
function normalizeFireRow(
  ...
  const dispatchDisplayTime =
    formatFireDispatchTime(
      dispatchTime
    )
